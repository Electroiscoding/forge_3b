# FORGE: A 1-Billion Parameter Recurrent-Gated Expert Architecture Enabling Full Pretraining, Fine-Tuning, and Post-Training Under $300
...
We introduce **FORGE** (**F**irst-principles **O**ptimal **R**ecurrent **G**ated **E**xpert), a high-efficiency 991.6-million-parameter ($\le \mathbf{1.0B}$ max total parameters) language model architecture designed from first principles with one dominant constraint: the entire lifecycle — pretraining, supervised fine-tuning (SFT), and direct preference optimization (DPO) — must complete within an ultra-lean compute budget on commercially available cloud GPU infrastructure. Standard transformer architectures at this scale require thousands of dollars of compute under Chinchilla-optimal training regimes, making them inaccessible to most independent researchers and small organizations. FORGE solves this by combining three novel architectural components: (1) **Adaptive Recurrent Gating (ARG)**, a hybrid recurrent-selective state space mechanism with learned interpolation between local precision and long-range compression; (2) **Hierarchical Sparse Expert (HSE) FFN**, a two-tier mixture-of-experts feed-forward network with 32 total experts per applicable layer but only 2 active; and (3) **Compound Positional Bias (CPB)**, a positional encoding scheme applied jointly to attention and recurrent state initialization. FORGE-1B has 991.6M total parameters ($\le 1.0\text{B}$) with 593.1M active parameters per forward pass, yielding superior throughput and cost efficiency. We provide exact hyperparameter tables, a phased training protocol, a data curation strategy, complete PyTorch implementations, optimizer configuration, and infrastructure setup instructions.
